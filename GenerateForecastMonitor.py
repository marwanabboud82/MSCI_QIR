...
def GenerateForecastMonitor(OutputUpdateSegmNbComp,OutputFinalInvestability):
    ...
    for mkt in OutputFinalInvestability.keys():
        ...
        Adds_List = ( (Market_SecurityData['Status']!='STD')  & (Market_SecurityData['Iter_Rank']<=QIR_MSnbC) & (Market_SecurityData['Final_Test']==1) & (Market_SecurityData['All_BasicTest']==1)).tolist()
        Market_SecurityData.loc[Adds_List,'Final_Monitor1']='Add'
        
        Dels_List = ( (Market_SecurityData['Status']=='STD')  & ((np.isnan(Market_SecurityData['Iter_Rank'])) | (Market_SecurityData['Iter_Rank']> QIR_MSnbC) | (Market_SecurityData['Final_Test']==0)) ).tolist()
        ...
        #Monitor 2 New Companies
        NewBufferAdds_List = ( (Market_SecurityData['Status']!='STD')  & (Market_SecurityData['Status']!='SML')  & (Market_SecurityData['Final_FF_Dist']<=BufferLimit) & (Market_SecurityData['Final_Full_Dist']<=BufferLimit)).tolist()
        Market_SecurityData.loc[NewBufferAdds_List,'Final_Monitor1']='Buffer_Add'
                
        NewAdds_List = ( (Market_SecurityData['Status']!='STD')   & (Market_SecurityData['Status']!='SML')  & (Market_SecurityData['Final_Test']==1) & (Market_SecurityData['All_BasicTest']==1)).tolist()
        Market_SecurityData.loc[NewAdds_List,'Final_Monitor1']='Add'
